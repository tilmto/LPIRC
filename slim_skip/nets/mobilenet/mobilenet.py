...
def mobilenet_base(  # pylint: disable=invalid-name
  ...
  with _scope_all(scope, default_scope='Mobilenet'), \
    ...
    for i, opdef in enumerate(conv_defs['spec']):
      ...
      try:
        if i==3 or i==5 or i==6 or i==8 or i==9 or i==10 or i==12 or i==13 or i==15 or i==16:
          net_next = opdef.op(net, **params)
          mask = skip_layer(net,scope='skip_'+str(i))
          one_tensor = tf.ones_like(mask)
          net = mask*net_next+(one_tensor-mask)*net
          layer_masks.append(tf.reshape(mask,[-1]))
        else:
    ...
    flops_fix = flops['conv1']+flops['16']+flops['24_1']+flops['32_1']+flops['64_1']+flops['96_1']+flops['160_1']+flops['320']+flops['conv2']+flops['conv3']
    flops_dyn = flops['24']+flops['32']*2+flops['64']*3+flops['96']*2+flops['160']*2
    flops_sum = flops_fix+flops_dyn

    computation_cost = tf.reduce_mean(flops['24']*layer_masks[0]+flops['32']*tf.reduce_sum(layer_masks[1:3],axis=0)
                      +flops['64']*tf.reduce_sum(layer_masks[3:6],axis=0)
                      +flops['96']*tf.reduce_sum(layer_masks[6:8],axis=0)
                      +flops['160']*tf.reduce_sum(layer_masks[8:],axis=0))+flops_fix
    ...
    return net, end_points,computation_cost


lstm_cell=tf.nn.rnn_cell.LSTMCell(num_units=10,num_proj=1,name='lstm_cell')
def skip_layer(feature_map,scope='skip_layer'):
    x = feature_map
    with tf.variable_scope(scope):
      ...
      x,state = tf.nn.static_rnn(lstm_cell,inputs=tf.unstack(x,axis=1),dtype=tf.float32,scope='rnn')

      x = tf.reshape(x[0],[-1,1,1,1])
...
def mobilenet(inputs,
  ...
  with tf.variable_scope(scope, 'Mobilenet', reuse=reuse) as scope:
    ...
    net, end_points ,computation_cost = mobilenet_base(inputs, scope=scope, **mobilenet_args)
    if base_only:
  ...
  return logits, end_points ,computation_cost
